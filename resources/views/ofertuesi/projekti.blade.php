...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4><a href="{{ route('departamentishitjes.ofertuesi.dashboard') }}"><i class="ri-arrow-left-fill"></i> Back</a></h4>
...
<script>
    function saveOffertPrice(id) {
        const input = document.getElementById('inputOffert' + id);
        const newValue = parseFloat(input.value);

        if (isNaN(newValue)) {
            alert("Ju lutem vendosni nje vlere numerike.");
            return;
        }

        fetch("{{ route('product.updateOffertPrice') }}", {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
                id: id,
                offert_price: newValue
            })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const modal = bootstrap.Modal.getInstance(document.getElementById('offertModal' + id));
                modal.hide();

                Swal.fire({
                    title: 'Ruajtur!',
                    text: 'Çmimi u përditësua me sukses.',
                    icon: 'success',
                    confirmButtonText: 'OK'
                }).then(() => {
                    // Reload DataTable without reloading the full page
                    $('#model-datatables').DataTable().ajax.reload(null, false);
                    $('#model-datatables2').DataTable().ajax.reload(null, false);
                });
            }else {
                alert(data.message || "Gabim gjatë ruajtjes së të dhënave.");
            }
        })
        .catch(() => {
    ...
    $(function() {
        ...
        function initializeDataTable(itemId, itemName) {
            ...
            $('.model-datatables').DataTable({
                ...
                ordering: false,
                ajax: {
                    url: "{{ route('ofertuesi.preorder.list_preorder', ['id' => $id]) }}",
                    data: function(d) {
                ...
                columns: [
                    ...
                    { data: 'offert_price', name: 'offert_price', orderable: false },
    ...
    $(function() {
        ...
        function initializeDataTable(itemId, itemName) {
            ...
            $('.model-datatables2').DataTable({
                ...
                ordering: false,
                ajax: {
                    url: "{{ route('ofertuesi.preorder.product_list', ['id' => $id]) }}",
                    data: function(d) {
                        d.name = $('#search-client').val();
                        d.status = $('#search-status').val();
                        d.date = $('#search-date').val();
